...
class Trainer(object):
    ...
    def _train_epoch(self, train_data):
        ...
        if self.config['mu'] > 0:
            if self.use_ddp:
                self.model.module.update_center()
            else:
                self.model.update_center()

        for batch in train_data:
            x_batch = batch[0].to(self.config['device'])
            self.optimizer.zero_grad()
            recon_x, quant_loss, count = self.model(x_batch)
            target,_ = torch.split(x_batch,
                                   [recon_x.shape[-1], self.config["cf_emb_dim"]], dim=1)
            reconstruction_mse_loss = F.mse_loss(recon_x, target, reduction='mean')
