...
def parse_args():
    ...
    parser.add_argument('--dataset', type=str, default='Sports_and_Outdoors', help='Dataset name')
...
if __name__ == '__main__':
    ...
    cf_emb_path = os.path.join(
        config['data_dir'], f"{config['dataset']}.cf_emb"
    )
    if os.path.exists(cf_emb_path):
        log(f'[TOKENIZER] Loading CF embeddings from {cf_emb_path}...', config['accelerator'], logger)
        cf_embs = np.fromfile(cf_emb_path, dtype=np.float32).reshape(-1, config['cf_emb_dim'])
    else:
        raise RuntimeError(f'[TOKENIZER] CF embeddings not found.')
    log(f'[TOKENIZER] CF embeddings shape: {cf_embs.shape}', config['accelerator'], logger)
    ...
    cf_embs = cf_embs[train_item_list]
    log(f'[TOKENIZER] Sentence embeddings shape after filtering: {sent_embs.shape}', config['accelerator'], logger)
    log(f'[TOKENIZER] CF embeddings shape after filtering: {cf_embs.shape}', config['accelerator'], logger)


    # Generate semantic IDs
    sent_embs = torch.FloatTensor(sent_embs)
    cf_embs = torch.FloatTensor(cf_embs)
    emb_data = torch.cat([sent_embs, cf_embs], dim=1)
